...
public class UserService {
	...
	/*@Transactional //o nosso Try-Catch (roolback por baixo dos panos)
	public ResponseEntity<Object> save(User user) {
		if (Objects.nonNull(user.getName()) && Objects.nonNull(user.getCpf())) {	
			if(UserRepository.existsByCpf(user.getCpf())) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("CPF ALREDY EXISTS");
			} else {
	...
	public ResponseEntity<Object> saveUserDto(@Valid UserDto userDto) {
		if (!userRepository.existsByCpf(userDto.getCpf())) {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("CPF alredy registred in use to system.");
		}
	}
